...
GLOBAL_ARGS="--networkid $NETID --identity $IDENTITY --unlock 0 --password passwords.txt --raft --rpc --rpcaddr 0.0.0.0 --rpcapi admin,db,eth,debug,miner,net,shh,txpool,personal,web3,quorum --emitcheckpoints"
...
#     fi
# fi
if [[ "$CURRENT_HOST_IP" == "10.0.0.2" ]]; then
    PRIVATE_CONFIG=~/blockcheq/data/constellation/constellation.conf nohup geth --datadir ~/blockcheq/data $GLOBAL_ARGS --permissioned --raftport 50401 --rpcport 22000 --port 21000 --mine --minerthreads 1 --syncmode full --unlock 0 --password ~/blockcheq/data/passwords.txt 2>> ~/blockcheq/logs/quorum_"${_TIME}".log &
else
    PRIVATE_CONFIG=~/blockcheq/data/constellation/constellation.conf nohup geth --datadir ~/blockcheq/data $GLOBAL_ARGS --permissioned --raftport 50401 --rpcport 22000 --port 21000 --mine --minerthreads 1 --syncmode full 2>> ~/blockcheq/logs/quorum_"${_TIME}".log &
fi
